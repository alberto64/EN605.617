...
*/
__global__ void addConstCUDA(const int *threadCountList, const int *randNumList, int *resultList) { 
...
* multithreading for index calculation using constant memory.
*/
__global__ void subConstCUDA(const int *threadCountList, const int *randNumList, int *resultList) { 
	const int idx = threadIdx.x + (blockIdx.x * blockDim.x); 
	resultList[idx] = threadCountList[idx] - randNumList[idx]; 
}
...
* multithreading for index calculation using constant memory.
*/
__global__ void multConstCUDA(const int *threadCountList, const int *randNumList, int *resultList) { 
	const int idx = threadIdx.x + (blockIdx.x * blockDim.x); 
	resultList[idx] = threadCountList[idx] * randNumList[idx]; 
}
...
*/
__global__ void modConstCUDA(const int *threadCountList, const int *randNumList, int *resultList) { 
	const int idx = threadIdx.x + (blockIdx.x * blockDim.x); 
	resultList[idx] = threadCountList[idx] % randNumList[idx]; 
}
...
/**
* runSharedOperations: Taking the number of blocks and threads it does 4 operations on the two 
* given arrays and prints their results. Uses shared memory.
*/
void runSharedOperations(int numBlocks, int totalThreads, int* threadCountList, int* randNumList) { 
...
/**
* runConstOperations: Taking the number of blocks and threads it does 4 operations on the two 
* given arrays and prints their results. Uses const memory
*/
void runConstOperations(const int numBlocks, const int totalThreads, const int* threadCountList, const int* randNumList) { 
...
int main(int argc, char** argv)
{
	...
	__constant__ int constNumBlocks = numBlocks;
    __constant__ int constTotalThreads = totalThreads; 
